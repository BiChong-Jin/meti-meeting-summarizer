...
class TestRegistration:
    def test_register_and_authenticate(self, test_db):
        register_user("user@digitalgrid.com", "password123")
        user = authenticate("user@digitalgrid.com", "password123")
        assert user["email"] == "user@digitalgrid.com"

    def test_first_user_gets_admin_role(self, test_db):
        register_user("admin@digitalgrid.com", "password123")
        user = authenticate("admin@digitalgrid.com", "password123")
        assert user["role"] == "admin"

    def test_second_user_gets_user_role(self, test_db):
        register_user("admin@digitalgrid.com", "password123")
        register_user("user2@digitalgrid.com", "password123")
        user = authenticate("user2@digitalgrid.com", "password123")
        assert user["role"] == "user"

    def test_rejects_wrong_domain(self, test_db):
        with pytest.raises(AuthError, match="@digitalgrid.com"):
    ...
    def test_rejects_short_password(self, test_db):
        with pytest.raises(AuthError, match="8文字以上"):
            register_user("user@digitalgrid.com", "short")

    def test_rejects_duplicate_email(self, test_db):
        register_user("user@digitalgrid.com", "password123")
        with pytest.raises(AuthError, match="既に登録"):
            register_user("user@digitalgrid.com", "password456")
    ...
    def test_allowed_domain_check_is_case_insensitive(self, test_db, monkeypatch):
        monkeypatch.setattr("auth.ALLOWED_DOMAIN", "DigitalGrid.COM")
        register_user("user@digitalgrid.com", "password123")
        assert authenticate("user@digitalgrid.com", "password123")
...
class TestAuthentication:
    def test_wrong_password_rejected(self, test_db):
        ...
        with pytest.raises(AuthError, match="正しくありません"):
            authenticate("user@digitalgrid.com", "wrongpassword")

    def test_nonexistent_user_rejected(self, test_db):
        with pytest.raises(AuthError, match="正しくありません"):
            authenticate("nobody@digitalgrid.com", "password123")


class TestAdminFunctions:
    def test_list_users(self, test_db):
        register_user("a@digitalgrid.com", "password123")
        register_user("b@digitalgrid.com", "password123")
        users = list_users()
        assert len(users) == 2
        emails = {u["email"] for u in users}
        assert emails == {"a@digitalgrid.com", "b@digitalgrid.com"}
    ...
    def test_delete_user(self, test_db):
        register_user("a@digitalgrid.com", "password123")
        register_user("b@digitalgrid.com", "password123")
        delete_user("b@digitalgrid.com")
        users = list_users()
        assert len(users) == 1
        assert users[0]["email"] == "a@digitalgrid.com"

    def test_delete_user_prevents_login(self, test_db):
        register_user("a@digitalgrid.com", "password123")
        delete_user("a@digitalgrid.com")
        with pytest.raises(AuthError, match="正しくありません"):
            authenticate("a@digitalgrid.com", "password123")
